...
catch_trials_data <- read_csv("catch_trials_data.csv")
followup_response_data <- read_csv("followup_response_data.csv")
...
## MH: shouldn't we be removing participants who fail the attention checks before computing summary statistics? 
CIs <- trials_data %>%
  group_by(item_presentation_condition, n_examples) %>%
  tidyboot_mean(column = response)
```
MH: if you print out `CIs`, there seem to be 62 entries for the accidental condition for 2 examples. is that correct?
